...
(define (f2-array-ref A i j)
  (let ((m1 (array-length A 0))
        (n1 (array-length A 1))
        (i1 (- i (array-start A 0)))
        (j1 (- j (array-start A 1))))
    (unless (and (>= i1 0) (>= j1 0) (< i1 m1) (< j1 n1))
      (error "invalid index value"))
    (f64vector-ref  (slot-ref A 'backing-storage) (+ (* i1 n1) j1))))
...
(define (f2-array-set! A i j d)
  (let ((m1 (array-length A 0))
    ...
    (unless (and (>= i1 0) (>= j1 0) (< i1 m1) (< j1 n1))
      (error "invalid index value"))
    (f64vector-set! (slot-ref A 'backing-storage) (+ (* i1 n1) j1) d)))
...
;; 行列の生成(2次元のみ)(キャッシュ使用)
(define (make-f2-array ms me ns ne :optional (maybe-init 0))
  (if use-f2-array-cache
    ...
    ((with-module gauche.array %make-array-internal-orig)
     <f64array> (shape ms me ns ne) maybe-init)))
...
;; 行列の初期化データ付き生成(2次元のみ)
(define (f2-array ms me ns ne . inits)
  (rlet1 ar (make-f2-array ms me ns ne 0)
...
(define (make-array-internal class shape . maybe-init)
  (if (and (with-module f2arrmat use-f2-array-cache)
    ...
    (receive (Vb Ve) (shape->start/end-vector shape)
      (let ((ms (s32vector-ref Vb 0))
            (me (s32vector-ref Ve 0))
            (ns (s32vector-ref Vb 1))
            (ne (s32vector-ref Ve 1)))
        (apply (with-module f2arrmat make-f2-array) ms me ns ne maybe-init)))
...
(define f2-array-trace
  (if *eigenmat-loaded*
    ...
    (lambda (ar1)
      ...
      (let ((m1  (array-length ar1 0))
            (n1  (array-length ar1 1))
            (v1  (slot-ref ar1 'backing-storage))
            (ret 0))
        (dotimes (i1 (min m1 n1))
          (inc! ret (f64vector-ref v1 (+ (* i1 n1) i1))))
        ret))))
...
(define f2-array-identity
  (if *eigenmat-loaded*
    ...
    (lambda (m1 n1)
      ...
      (let* ((ar1 (make-f2-array 0 m1 0 n1 0))
             (v1  (slot-ref ar1 'backing-storage)))
        (dotimes (i1 (min m1 n1))
          (f64vector-set! v1 (+ (* i1 n1) i1) 1))
        ar1))))
    ...
    (lambda (ar1 i1r)
      ...
      (let ((m1 (array-length ar1 0))
        ...
        (unless (and (>= i1 0) (< i1 m1))
          (error "invalid index value"))
        (let* ((start (* i1 n1))
               (ar2   (make-f2-array 0 1 0 n1)) ; 結果は 1 x n1 になる
               (v2    (slot-ref ar2 'backing-storage)))
          (f64vector-copy! v2 0 v1 start (+ start n1))
          ar2)))))
  ...
  (if *eigenmat-loaded*
    ...
    (lambda (ar2 ar1 i1r)
      ...
      (let ((m1 (array-length ar1 0))
            ...
            (i1 (- i1r (array-start ar1 0)))
            (v1 (slot-ref ar1 'backing-storage))
            (m2 (array-length ar2 0))
            (n2 (array-length ar2 1))
            (v2 (slot-ref ar2 'backing-storage)))
        (unless (and (>= i1 0) (< i1 m1))
          (error "invalid index value"))
    ...
    (lambda (ar1 j1r)
      ...
      (let ((m1 (array-length ar1 0))
        ...
        (unless (and (>= j1 0) (< j1 n1))
          (error "invalid index value"))
        (let* ((ar2 (make-f2-array 0 m1 0 1))   ; 結果は m1 x 1 になる
               (v2  (slot-ref ar2 'backing-storage)))
          (dotimes (i2 m1)
            (f64vector-set! v2 i2 (f64vector-ref v1 (+ j1 (* i2 n1)))))
          ar2)))))
  ...
  (if *eigenmat-loaded*
    ...
    (lambda (ar1 i1r j1r m2 n2)
      ...
      (let ((m1 (array-length ar1 0))
        ...
        (let* ((ar2 (make-f2-array 0 m2 0 n2))  ; 結果は m2 x n2 になる
               (v2  (slot-ref ar2 'backing-storage)))
          (dotimes (i2 m2)
            (let1 start (+ (* (+ i1 i2) n1) j1)
              (f64vector-copy! v2 (* i2 n2) v1 start (+ start n2))))
          ar2)))))
  ...
  (if *eigenmat-loaded*
    ...
    (lambda (ar2 ar1 i1r j1r m2 n2)
      ...
      (let ((m1  (array-length ar1 0))
            ...
            (j1  (- j1r (array-start ar1 1)))
            (v1  (slot-ref ar1 'backing-storage))
            (m2a (array-length ar2 0))
            (n2a (array-length ar2 1))
            (v2  (slot-ref ar2 'backing-storage)))
        (unless (and (>= m2 0) (>= n2 0))
          (error "invalid block size"))
        (unless (and (>= i1 0) (>= j1 0) (<= (+ i1 m2) m1) (<= (+ j1 n2) n1))
...
(define f2-array-block-copy
  (if *eigenmat-loaded*
    ...
    (lambda (ar1 i1r j1r m3 n3 ar2 i2r j2r)
      (check-array-type ar1 ar2)
      (check-array-rank ar1 ar2)
      (let ((m1 (array-length ar1 0))
            (n1 (array-length ar1 1))
        ...
        (let* ((ar3 (f2-array-copy ar2))        ; 結果は m2 x n2 になる
               (v3  (slot-ref ar3 'backing-storage)))
          (dotimes (i3 m3)
            (let ((start1 (+ (* (+ i1 i3) n1) j1))
                  (start3 (+ (* (+ i2 i3) n2) j2)))
              (f64vector-copy! v3 start3 v1 start1 (+ start1 n3))))
          ar3)))))
...
(define f2-array-block-copy!
  (if *eigenmat-loaded*
    eigen-array-block-copy!
    (lambda (ar3 ar1 i1r j1r m3 n3 ar2 i2r j2r)
      ...
      (let ((m1  (array-length ar1 0))
            ...
            (j2  (- j2r (array-start ar2 1)))
            (v2  (slot-ref ar2 'backing-storage))
            (m3a (array-length ar3 0))
            (n3a (array-length ar3 1))
            (v3  (slot-ref ar3 'backing-storage)))
        (unless (and (>= m3 0) (>= n3 0))
          (error "invalid block size"))
        (unless (and (>= i1 0) (>= j1 0) (<= (+ i1 m3) m1) (<= (+ j1 n3) n1))
